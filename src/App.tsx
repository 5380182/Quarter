      ...
      {page==='storybook' && (
        <div style={{position:'fixed',inset:0,zIndex:2147483647,background:(storyView==='read' ? (storyReadBg || storyBg) : storyBg)?(safeStoryBgSize==='repeat'?`url(${storyView==='read' ? (storyReadBg || storyBg) : storyBg}) center/300px auto repeat`:`url(${storyView==='read' ? (storyReadBg || storyBg) : storyBg}) center/${safeStoryBgSize} no-repeat`):'#fff8f3',display:'flex',flexDirection:'column',height:'100dvh',overflow:'hidden'}}>
          ...
          <div style={{padding:'18px 16px 28px',display:'flex',flexDirection:'column',gap:12,position:'relative',zIndex:2,overflowY:'auto',flex:1}}>
            ...
            {storyView==='read' && (()=>{ const chs=safeChapters.filter(c=>c.story_id===storyId).sort((a,b)=>a.chapter_number-b.chapter_number); const ch=chs[storyChIdx]; if(!ch)return <div style={{textAlign:'center',padding:40,color:'#999'}}>没有章节</div>; return <div style={{background:'rgba(255,255,255,0.92)',borderRadius:14,padding:'18px 16px 22px',border:'1px solid rgba(0,0,0,0.05)',boxShadow:'0 8px 24px rgba(90,62,43,0.04)'}}><div style={{fontFamily:"'LXGW WenKai',serif",fontSize:14,color:'var(--accent)',marginBottom:6,fontWeight:500}}>{ch.title}</div><div style={{display:"flex",gap:8,marginBottom:8}}><button onClick={()=>startEditChapter(ch)} style={{background:"none",border:"1px solid rgba(0,0,0,0.1)",borderRadius:6,padding:"4px 12px",fontSize:11,color:"#888",cursor:"pointer"}}>编辑</button><button onClick={()=>{if(confirm("删除这个篇章？"))deleteChapter(ch.id)}} style={{background:"none",border:"1px solid rgba(0,0,0,0.1)",borderRadius:6,padding:"4px 12px",fontSize:11,color:"#c88",cursor:"pointer"}}>删除</button></div><div style={{fontFamily:"'Noto Sans SC',sans-serif",fontSize:14,color:'#3c3c3c',lineHeight:'2.2',letterSpacing:'0.5px'}}>{ch.content.split('\n').map((p,i)=><p key={i} style={{textIndent:'2em',margin:'0 0 8px 0'}}>{p}</p>)}</div><div style={{marginTop:24,padding:'16px 0',borderTop:'1px solid rgba(0,0,0,0.06)'}}><div style={{fontFamily:"'LXGW WenKai',serif",fontSize:13,color:'#8a7a6a',marginBottom:8}}>rating</div><div style={{display:'flex',gap:4,marginBottom:12}}>{[1,2,3,4,5].map(n=><span key={n} onClick={()=>setChRating(n)} style={{cursor:'pointer',fontSize:22,color:n<=chRating?'#d4a574':'#ddd',transition:'color 0.2s'}}>{n<=chRating?'\u2605':'\u2606'}</span>)}</div><textarea value={chComment} onChange={e=>setChComment(e.target.value)} placeholder="write something..." rows={2} style={{width:'100%',border:'1px solid rgba(0,0,0,0.08)',borderRadius:8,padding:'8px 12px',fontSize:12,fontFamily:"'Noto Sans SC',sans-serif",resize:'none',background:'rgba(255,255,255,0.6)',color:'#3c3c3c',outline:'none',boxSizing:'border-box'}} /><div style={{display:'flex',gap:8,marginTop:8}}><button onClick={()=>submitRating(ch.id)} style={{background:'var(--accent)',border:'none',borderRadius:6,padding:'6px 16px',fontSize:11,color:'white',cursor:'pointer',opacity:chRating<1?0.4:1}}>submit</button><button onClick={()=>{submitNudge(ch.story_id);}} style={{background:'none',border:'1px solid rgba(0,0,0,0.1)',borderRadius:6,padding:'6px 16px',fontSize:11,color:'#888',cursor:'pointer'}}>催更 {nudgeCounts[ch.story_id]?'('+nudgeCounts[ch.story_id]+')':''}</button><button onClick={()=>setShowTip(true)} style={{background:'none',border:'1px solid rgba(212,165,116,0.4)',borderRadius:6,padding:'6px 16px',fontSize:11,color:'#d4a574',cursor:'pointer'}}>reward</button></div>{showTip&&<div style={{marginTop:10,display:'flex',gap:6,alignItems:'center'}}><input type="number" value={tipAmount} onChange={e=>setTipAmount(e.target.value)} placeholder="amount" style={{width:80,border:'1px solid rgba(0,0,0,0.1)',borderRadius:6,padding:'6px 10px',fontSize:12,outline:'none'}} /><button onClick={()=>submitTip(ch.title)} style={{background:'#d4a574',border:'none',borderRadius:6,padding:'6px 14px',fontSize:11,color:'white',cursor:'pointer'}}>confirm</button><button onClick={()=>setShowTip(false)} style={{background:'none',border:'none',fontSize:11,color:'#aaa',cursor:'pointer'}}>cancel</button></div>}{chRatings.length>0&&<div style={{marginTop:14,borderTop:'1px solid rgba(0,0,0,0.04)',paddingTop:10}}>{chRatings.map((r: any,i: number)=><div key={i} style={{fontSize:11,color:'#999',marginBottom:6}}><span style={{color:'#d4a574'}}>{'\u2605'.repeat(r.rating)}</span>{r.comment&&<span style={{marginLeft:6}}>{r.comment}</span>}</div>)}</div>}</div><div style={{display:'grid',gridTemplateColumns:'1fr auto 1fr',alignItems:'center',gap:10,marginTop:40,paddingTop:20,borderTop:'1px solid rgba(0,0,0,0.06)'}}>{storyChIdx>0?<button onClick={()=>setStoryChIdx(storyChIdx-1)} style={{justifySelf:'start',background:'none',border:'1px solid rgba(0,0,0,0.1)',borderRadius:8,padding:'8px 18px',fontSize:12,color:'#666',cursor:'pointer'}}>上一篇</button>:<div /> }<div onClick={()=>{setNewStoryTitle('');setNewStoryContent('');setNewStoryCat('');setShowStoryForm(true)}} style={{display:'flex',alignItems:'center',justifyContent:'center',cursor:'pointer',color:'#b08f76'}}><div style={{fontSize:26,lineHeight:1,fontFamily:'Cormorant Garamond, serif'}}>+</div></div>{storyChIdx<chs.length-1?<button onClick={()=>setStoryChIdx(storyChIdx+1)} style={{justifySelf:'end',background:'var(--accent)',border:'none',borderRadius:8,padding:'8px 18px',fontSize:12,color:'white',cursor:'pointer'}}>下一篇</button>:<div />}</div></div>})()}
