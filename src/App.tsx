      ...
      {page==='storybook' && (
        <div style={{position:'fixed',inset:0,zIndex:2147483647,background:storyBg?(safeStoryBgSize==='repeat'?`url(${storyBg}) center/300px auto repeat`:`url(${storyBg}) center/${safeStoryBgSize} no-repeat`):'#fff8f3',display:'flex',flexDirection:'column',height:'100dvh',overflow:'hidden'}}>
          ...
          <div style={{padding:'18px 16px 28px',display:'flex',flexDirection:'column',gap:12,position:'relative',zIndex:2,overflowY:'auto',flex:1}}>
            ...
            {storyView==='read' && (()=>{ const chs=safeChapters.filter(c=>c.story_id===storyId).sort((a,b)=>a.chapter_number-b.chapter_number); const ch=chs[storyChIdx]; if(!ch)return <div style={{textAlign:'center',padding:40,color:'#999'}}>没有章节</div>; return <div style={{background:'rgba(255,252,248,0.50)',backdropFilter:'blur(12px)',WebkitBackdropFilter:'blur(12px)',borderRadius:18,padding:'18px 16px 22px',border:'1px solid rgba(255,255,255,0.24)',boxShadow:'0 8px 24px rgba(90,62,43,0.08)'}}><div style={{fontFamily:"'LXGW WenKai',serif",fontSize:14,color:'var(--accent)',marginBottom:6,fontWeight:500}}>{ch.title}</div><div style={{fontFamily:"'Noto Sans SC',sans-serif",fontSize:14,color:'#3c3c3c',lineHeight:'2.2',letterSpacing:'0.5px'}}>{ch.content.split('\n').map((p,i)=><p key={i} style={{textIndent:'2em',margin:'0 0 8px 0'}}>{p}</p>)}</div><div style={{display:'flex',justifyContent:'space-between',marginTop:40,paddingTop:20,borderTop:'1px solid rgba(0,0,0,0.06)'}}>{storyChIdx>0?<button onClick={()=>setStoryChIdx(storyChIdx-1)} style={{background:'none',border:'1px solid rgba(0,0,0,0.1)',borderRadius:8,padding:'8px 20px',fontSize:12,color:'#666',cursor:'pointer'}}>上一篇</button>:<div/>}{storyChIdx<chs.length-1&&<button onClick={()=>setStoryChIdx(storyChIdx+1)} style={{background:'var(--accent)',border:'none',borderRadius:8,padding:'8px 20px',fontSize:12,color:'white',cursor:'pointer'}}>下一篇</button>}</div><div onClick={()=>{setNewStoryTitle('');setNewStoryContent('');setNewStoryCat('');setShowStoryForm(true)}} style={{display:'flex',alignItems:'center',justifyContent:'center',padding:'10px 0 0',cursor:'pointer',color:'#b08f76'}}><div style={{fontSize:26,lineHeight:1,fontFamily:'Cormorant Garamond, serif'}}>+</div></div></div>})()}
